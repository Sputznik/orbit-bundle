	...
	class ORBIT_POST_TYPE extends ORBIT_BASE{
		...
		function register_types_from_db(){
			...
			if( $query->have_posts() ){
				...
				while( $query->have_posts() ){
					...
					$show_in_rest = get_post_meta( $query->post->ID, 'show_in_rest', true );
					...
					$meta_fields_to_be_appended = apply_filters( 'orbit_post_type_meta_fields_appended', array( 'menu_icon', 'supports', 'show_in_rest' ) );
					foreach( $meta_fields_to_be_appended as $meta_field ){
						if( $meta_field != 'show_in_rest' ){
							$temp[ $meta_field ] = get_post_meta( $query->post->ID, $meta_field, true );
						}
						else{
							$temp['show_in_rest'] = ( isset( $show_in_rest ) && $show_in_rest ) ? ( in_array( 'show_post', $show_in_rest ) ? true : false ) : false;
						}
					}
						...
						if( isset( $post_term->slug ) && isset( $post_term->name ) ){
							...
							if( !isset( $terms[ $post_term->slug ] ) ){
								$terms[ $post_term->slug ] = array(
																...
																'post_types'	=> array(),
																'show_in_rest' => ( isset( $show_in_rest ) && $show_in_rest ) ? ( in_array( 'show_taxonomy', $show_in_rest ) ? true : false ) : false
															);
							}
		...
		function create_taxonomy( $taxonomy ) {
			...
			register_taxonomy( $r['slug'], $taxonomy['post_types'], array(
				...
				'show_in_rest'		=> isset( $r['show_in_rest'] ) && $r['show_in_rest'] ? true : false
			));
